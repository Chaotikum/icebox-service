...
exports.getAllConsumers = function(client, callback) {
  var query = client.query(
    "SELECT username, avatarmail, ledger, vds, lastchange " +
    "FROM consumers " +
...
exports.insertNewConsumer = function(client, data, callback) {
  console.log(data);
  var query = client.query(
    "INSERT INTO consumers (username, ledger, avatarmail, vds, lastchange) " +
    "VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP)", [data.username, 0, "", false]);
...
exports.manipulateConsumer = function(client, data, callback) {
  if (data.username == "Anon") {
    ...
    var query = client.query("UPDATE consumers SET avatarmail = ($1), vds = ($2), lastchange = CURRENT_TIMESTAMP WHERE username = ($3)", [data.avatarmail, data.vds, data.username]);
...
exports.addDeposit = function(client, username, amount, callback) {
  ...
  if (username == "Anon") {
    ...
    var query = client.query("UPDATE consumers SET ledger = ledger + ($2), lastchange = CURRENT_TIMESTAMP WHERE username = ($1)", [username, amount]);
    query.on('end', function() {
      exports.getConsumersByName(client, username, callback);
    });
  }
